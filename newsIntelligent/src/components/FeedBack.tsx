...
function FeedBack() {
    ...
    const handleSende = async () => {
        ...
        try {
            await postFeedback(); // tag, inputText 등 필요한 정보 전달하도록 수정 가능
            ...
            {openFeedback && (
                <div className="flex flex-col w-[380px] shadow-[2px_4px_8px_0px_rgba(0,0,0,0.15)] backdrop-blur-lg rounded-t-[16px]">
                    ...
                    <div className={`flex flex-col w-full h-[511px]`}>
                        ...
                        <textarea
                            className="w-full h-[121px] bg-white border border-[#E6E6E6] p-[12px] relative focus:outline-none text-sm placeholder:text-[#919191] font-normal leading-none resize-none"
                            placeholder="메세지 입력"
                            value={inputText}
                            onChange={e => setInputText(e.target.value)}
                        />
                        <button
                            className="w-[59px] h-[30px] rounded-[4px] px-[16px] py-[6px] bg-[#0EA6C0]
