...
function Notification({
  ...
  const [check, setCheck] = useState(false); // 네가 추가한 부분
  ...
  const handleCheck = () => {
    ...
    setCheck(true); // 네가 필요한 처리일 수 있어 보존함
  };
  ...
  return (
    ...
    >
      ...
      <ul>
        <li
          className="flex justify-between h-11 gap-3 px-[12px] py-[6px] 
          border-b-2 border-b-[#E6E6E6]"
        >
          <div className="flex justify-start items-center gap-2.5">
            <div
              className="w-14 h-5 px2.5 bg-[#0EA6C01A] rounded-[20px] 
              text-center text-[#0B8E8E] text-xs font-semibold"
            >
              {" "}
              구독{" "}
            </div>
            <div className="justify-center text-sm">
              <span className="text-[#0B8E8E]">'SKT 유심'</span>의 새로운
              업데이트가 있습니다.
            </div>
          </div>
          <div className="w-8 text-right justify-center text-500 text-xs">
            23:08
          </div>
        </li>
        <li
