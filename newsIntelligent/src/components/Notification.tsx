...
function Notification({ isOpen, setNotification, onClose }: { isOpen: boolean, setNotification: (value: boolean) => void, onClose?: () => void }) {
  ...
  */
  ...
  useEffect(() => {
    if (!isOpen) return; // 알림이 닫혀있을 때는 작동 X

    // 컴포넌트가 완전히 마운트된 후에 이벤트 리스너 등록
    const timer = setTimeout(() => {
      const handleClickOutside = (e: MouseEvent) => {
        const target = e.target as Node;
        if (notificationRef.current && !notificationRef.current.contains(target)) { // 알림 영역 외부 클릭 감지
          // 부모 컴포넌트의 상태를 직접 변경하지 않고 onClose 콜백만 호출
          onClose?.();
          e.preventDefault();
          e.stopPropagation();
        }
