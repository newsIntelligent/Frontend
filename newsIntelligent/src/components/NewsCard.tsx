...
const getImageSourceLink = (related: any[], data: TopicWithSub): string | null => {
  ...
  const byPress = related.find(
    (c: any) => (c?.press ?? '').trim().toLowerCase() === press
  )?.newsLink
  if (byPress) return byPress

  const byImageUrl = related.find(
    (c: any) => (c as any)?.imageUrl && (c as any).imageUrl === img
  )?.newsLink
            ...
            <div className="flex flex-1 justify-between items-center">
              <p className="w-[298px] h-[21px] text-[12px] font-normal text-[#919191] overflow-hidden whitespace-nowrap truncate">
                ...
                {newsLink ? (
                  ...
                  <span className="underline underline-offset-2 decoration-[#919191]">
                    {sourceTitle}
                  </span>
                )}
              </p>
              ...
              <div className="flex flex-col gap-2">
                {isLoading && (
                  <div className="flex flex-col ml-5 justify-center w-[319px] h-[89px]">
                    <span className="pl-4 text-[12px] text-[#919191]">
                      출처 기사를 불러오는 중...
                    </span>
                  </div>
                )}
                ...
                {!isLoading &&
                  ...
                  (relatedList.length === 0 ? (
                    ...
                    relatedList.map((c: any) => (
                      <div
                        key={c.id}
                        className="flex flex-col ml-5 justify-center w-[319px] h-[89px]"
                      >
